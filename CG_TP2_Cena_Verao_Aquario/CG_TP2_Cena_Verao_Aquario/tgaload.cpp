...
void tgaPutPacketTuples ( image_t *p, unsigned char *temp_colour,
															 int &current_byte )
{
...
void tgaGetAPacket ( int &current_byte, image_t *p, FILE *file )
{
   ...
   {
      fread ( temp_colour, ( sizeof ( unsigned char )* p->info.components ),
																	  1, file );
...
void tgaUploadImage ( image_t *p, tgaFLAG mode )
{
   ...
   if ( !( mode&TGA_NO_MIPMAPS ))
      gluBuild2DMipmaps ( GL_TEXTURE_2D, internal_format, p->info.width,
                          p->info.height, p->info.tgaColourType,
													GL_UNSIGNED_BYTE, p->data );
   else
      glTexImage2D ( GL_TEXTURE_2D, 0, internal_format, p->info.width,
                     p->info.height, 0, p->info.tgaColourType, GL_UNSIGNED_BYTE,
																	  p->data ); 
}
...
void tgaGetImageHeader ( FILE *file, tgaHeader_t *info )
{
   /*   Stupid byte alignment means that we have to fread each field
        ...
        1 as it should be.  Grrr.  Gotta love optimising compilers - theres a
        pragma for Borland, but I dunno the number for MSVC or GCC :(
    */
